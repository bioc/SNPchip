...
A hidden Markov models can be used to identify chromosomal alterations
using genotype and copy number estimates as described in the
\Rpackage{VanillaICE} vignette.

\section{Combining data objects using different annotation platforms}

Here we illustrate how one may combine different objects of class
\Robject{SnpCallSetPlus}.  Following the \Rpackage{oligo} vignette, I
created \Robject{hind} and \Robject{xba} instances of
\Robject{SnpCallSetPlus}:
