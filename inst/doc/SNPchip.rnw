...
in the featureData slot.  Annotation is provided for 10k, 100k, and
500k Affymetrix chips. See Section \ref{sec:annotation} for more
information.
...
Currently, we provide annotation via static tables that can be
downloaded from our website.  Bioconductor annotation packages for
high throughput SNP platforms are under development.  Column headers
for the annotation that is currently available for each SNP is here.
...
To use the entire data.frame for annotation, one could do the following:
<<addFeaturedata, eval=FALSE>>=
save(mapping50kXba240, "mapping50kXba240.rda", compress=TRUE)
save(mapping50kHind240, "mapping50kHind240.rda", compress=TRUE)
obj <- addFeatureData(annSnpset, path="./")
colnames(pData(featureData(obj)))
@ 
...
will automatically download the annotation from the website and store
it in the featureData slot.
