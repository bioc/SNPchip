...
\Rpackage{SNPchip} defines classes useful for organizing high
throughput genomic data.  The classes defined here extend the
...
data.  The classes and methods included in the \Rpackage{SNPchip} are
useful for summarizing and visualing SNP data, as well as providing a
