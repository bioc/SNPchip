...
<<gw.params>>=
graph.par <- getPar(sample.snpset[, 1:3], add.cytoband=FALSE)
graph.par$cex <- 2
graph.par$use.chromosome.size <- TRUE
...
<<graph.par3>>=
graph.par <- getPar(sample.snpset[chromosome(sample.snpset) %in% c(1, 7, 16, 19, "X"), 2])
graph.par$cex <- 0.8
...
graph.par$cex <- 0.8
graph.par$pch <- 20
graph.par$bg <- c("royalblue", "red", "royalblue")
graph.par$bty <- "o"
graph.par$cex.axis <- 1.2
graph.par$cex.lab <- 1.5
@ 
