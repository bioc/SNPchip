...
<<subsetting>>=
##source("~/madman/Rpacks/SNPchip/R/methods-ParESet.R")
##source("~/madman/Rpacks/SNPchip/R/initialize-methods.R")
...
<<plot1a, fig=TRUE, width=8, height=6>>=
plotSnp(graph.par, snpset)
@ 
...
graph.par <- getPar(graph.par, sample.snpset[, 1:3], add.cytoband=FALSE)
@ 
...
<<graph.par3>>=
graph.par <- new("ParSnpSet")
graph.par <- getPar(graph.par, sample.snpset[chromosome(sample.snpset) %in% c(1, 7, 16, 19, "X"), 2])
graph.par$cex <- 0.8
graph.par$mar <- rep(0.5, 4)
##graph.par$mar <- c(0.5, 0.5, 3, 0.5)
...
plotSnp(graph.par, sample.snpset[chromosome(sample.snpset) %in% c(1, 7, 16, 19, "X"), 2])
@ 
\clearpage
...
This is a static table and will be deprecated once chromosome-level
annotation is provided by Bioconductor.  If chromosome-level
annotation is available in Bioconductor, please send me a note.
