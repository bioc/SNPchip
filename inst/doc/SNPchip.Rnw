...
<<gw.params>>=
graph.par <- plotSnp(sample.snpset[, 1:3], add.cytoband=FALSE)
...
<<graph.par3>>=
graph.par <- plotSnp(sample.snpset[chromosome(sample.snpset) %in% c(1, 7, 16, 19, "X"), 2])
graph.par$cex <- 0.8
